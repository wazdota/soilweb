<template>
	<section>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			<el-table-column prop="id" label="管理员ID" width="100" sortable>
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="更改名称" :visible.sync="editFormVisible" :close-on-click-modal="false">
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				//NProgress.start();
				this.$axios.get('/v1/admins',{params:para}).then(response => {
			...
			handleDel: function (row) {
				this.$confirm('确认删除该用户吗?', '提示', {
					...
					//NProgress.start();
					let para = { id: row.id };
					this.$axios.delete("/v1/admin/"+row.id).then((res) => {
						this.listLoading = false;
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$axios.put('/v1/admin_s/'+this.editForm.id,para).then((res) => {
